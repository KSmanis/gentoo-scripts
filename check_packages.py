...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help="the path (file or directory) to check for package keywords"
             " (default: %(default)s)",
    )
    parser.add_argument(
        '--license-path',
        ...
        help="the path (file or directory) to check for package licenses"
             " (default: %(default)s)",
    )
    parser.add_argument(
        '--use-path',
        ...
        help="the path (file or directory) to check for package USE flags"
             " (default: %(default)s)",
    )
    parser.set_defaults(
    ...
    for file, line in _read_path(Path(path)):
        ...
        for package in installed_packages:
            try:
                ...
                _log(
                    "{file_name}: {atom} is no longer available in the portage"
                    " tree!",
                    file_name=file.name,
                    atom=atom,
                )
                ...
                for keyword in keywords:
                    if keyword not in available_keywords:
                        _log(
                            "{file_name}: {atom} no longer requires the"
                            " {highlight} keyword!",
                            file_name=file.name,
                            atom=atom,
                            highlight=keyword,
...
def check_use_flags(path=DEFAULT_USE_PATH,
                    check_sort_order=DEFAULT_CHECK_SORT_ORDER):
            ...
            try:
                ...
                for use_flag in map(_strip_use_flag, use_flags):
                    if use_flag not in available_use_flags:
                        _log(
                            "{file_name}: {atom} no longer makes use of the"
                            " {highlight} USE flag!",
                            file_name=file.name,
                            atom=atom,
                            highlight=use_flag,
